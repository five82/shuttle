...
- Running vs waiting comes from task state only (`QueueItem.runningTasks`); an item's `stage` is the scheduler's coarse position and lags during overlap windows.
